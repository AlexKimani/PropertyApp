		...
		case R.id.update:
			if( ( !user.getText().toString().equals("")) && ( !pass1.getText().toString().equals("")) && ( !pass2.getText().toString().equals(""))){
				if(p1.matches(p2)){
					...
					Toast.makeText(getApplicationContext(),"Passwords Mismatch please check again", Toast.LENGTH_SHORT).show(); 
				}
			}
			else{
				Toast.makeText(getApplicationContext(),"Enter all fields required", Toast.LENGTH_SHORT).show(); 
			}
		break;	
		}		 
	...
	private class updatepass extends AsyncTask<Object, Object, Object>{
		...
		String code1 = pass1.getText().toString();		
         ...
         protected void onPreExecute() {
             ...
             pDialog.setMessage("Updating Credentials...");
		...
		protected Object doInBackground(Object... params) {
			...
			try {
		      ...
		      HttpPost httppost = new HttpPost("http://10.0.2.2/Propertyapp/updatedata.php");
		        //use HTTPClient to execute the HTTPPost
		        // Execute HTTP Post Request
		      //encode URL
		      ArrayList<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(2);
		      nameValuePairs.add(new BasicNameValuePair("idnumber",id));
		      nameValuePairs.add(new BasicNameValuePair("passcode",code1));  
