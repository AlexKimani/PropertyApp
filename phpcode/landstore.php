...
if (!empty($_POST)) {
    ...
    if (empty($_POST['psize']) || empty($_POST['plocation'])|| empty($_POST['pname'])|| empty($_POST['price'])|| empty($_POST['username'])|| empty($_POST['ltitle'])) {
  ...
  $query = "INSERT INTO PROPERTY_TB(property_name,property_size,property_type,property_location,property_price,user_id) VALUES (:name, :size,:type,:place, :value,$field1)";
   ...
   /* $response["success"] = 1;
    ...
    $query3  = " SELECT property_id FROM PROPERTY_TB WHERE property_type = 'land' AND property_name=:nam";
    //now lets update what :user should be
    $query_params = array(
        ':nam' => $_POST['pname']
    );
...
?>
	<h1>Property</h1> 
	<form action="landstore.php" method="post"> 
	    Name:<br /> 
        ...
        <br /><br /> 
        Land Title:<br /> 
        <input type="text" name="ltitle" value="" /> 
        <br /><br /> 
