...
if (!empty($_POST)) {
    ...
    if (empty($_POST['psize']) || empty($_POST['plocation'])|| empty($_POST['pname'])|| empty($_POST['price'])|| empty($_POST['username'])|| empty($_POST['Bnumber'])) {
  ...
  $query = "INSERT INTO PROPERTY_TB(property_name,property_size,property_type,property_location,property_price,user_id) VALUES (:name, :size,:type,:place, :value,$field1)";
   ...
   /* $response["success"] = 1;
    ...
    $query3  = " SELECT property_id FROM PROPERTY_TB WHERE property_type = 'commercial_house' AND property_name=:nam";
    //now lets update what :user should be
    $query_params = array(
        ':nam' => $_POST['pname']
    );
...
?>
	<h1>Property</h1> 
	<form action="commstore.php" method="post"> 
	    Name:<br /> 
        ...
        <br /><br /> 
        Building Number:<br /> 
        <input type="text" name="Bnumber" value="" /> 
        <br /><br /> 
